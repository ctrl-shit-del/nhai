...
export function useCameraFrame(
  ...
  const sharedFaceRegion = useSharedValue<FaceRegion | null>(null);
  ...
  const updateDetection = useCallback((face: FaceRegion | null) => {
    sharedFaceRegion.value = face;
    sharedQuality.value    = face
      ? Number(
          (
            (face.confidence * 0.7) +
            (Math.min(1, (face.width * face.height) / 90_000) * 0.3)
          ).toFixed(3)
        )
      : 0;
  }, [sharedFaceRegion, sharedQuality]);
  ...
  const captureDetectedFace = useCallback((): FaceRegion | null => sharedFaceRegion.value, [sharedFaceRegion]);
